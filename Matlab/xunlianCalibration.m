...
classNames = ["background","Calibration"];
pxds = pixelLabelDatastore(gTruth);
...
pximds = pixelLabelImageDatastore(imds, pxds, 'OutputSize', [1920 1920 3],...
    'ColorPreprocessing', 'gray2rgb');

opts = trainingOptions("sgdm", 'ExecutionEnvironment', 'cpu',...
    'InitialLearnRate', 0.001, 'MiniBatchSize', 2, 'Plots',...
    'training-progress', 'MaxEpochs', 10);

[net, info] = trainNetwork(pximds, lgraph, opts);

save('Cal2resnet50-10.mat', 'net');
save('Cal2Inforesnet50-10.mat', 'info');
